...
export function Item({
  ...
  const index = useMemo(() => {
    return listContext.peers
      ? listContext.peers.indexOf(container.current)
      : -1;
  }, [listContext.peers]);

  const isActive = useMemo(
    () => index == listContext.activeItem,
    [listContext.activeItem, index],
  );
